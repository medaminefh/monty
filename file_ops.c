 ...
 * Return: void
 */
void find_func(char *opcode, char *v, int line_num, int format)
{
	int i;
	int flag;
	...
	for (flag = 1, i = 0; func_list[i].opcode != NULL; i++)
	{
		...
		{
			call_fun(func_list[i].f, opcode, v, line_num, format);
			flag = 0;
		}
	}
	if (flag == 1)
		p_err(3, line_num, opcode);
}


/**
 * call_fun - call the appropriet fun
 ...
 * if 1 nodes will be entered as a queue.
 */
void call_fun(op_func func, char *op, char *val, int line_num, int format)
{
	stack_t *node;
	int flag;
	int i;
